...
public class Convert {
	...
	public static final int byteArrayToInt_Little(byte byt[]) {
		...
		else if (byt.length == 2) return (0xff & byt[0]) | ((0xff & byt[1]) << 8);
		else if (byt.length >= 4) return (0xff & byt[0]) | (0xff & byt[1]) << 8 | (0xff & byt[2]) << 16 | (0xff & byt[3]) << 24;
		else return 0;
	}
	...
	public static final int byteArrayToInt_Big(byte byt[]) {
		...
		else if (byt.length >= 4) return (0xff & byt[0]) << 24 | (0xff & byt[1]) << 16 | (0xff & byt[2]) << 8 | 0xff & byt[3];
		else return 0;
	}
